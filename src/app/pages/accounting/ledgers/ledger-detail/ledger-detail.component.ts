...
// search_date_from
  ...
  ngOnInit(): void {
    this.activRoute.params.subscribe((res:any)=>{
      ...
      // route pagination update
      // route pagination update
      console.log(res)


      // this.get_all_entries(this.activeSlug, this.currentPage, this.itemsPerPage)
      this.load_data(this.activeSlug,this.currentPage, this.itemsPerPage)
  ...
  load_data(slug:string,currentPage:number,itemsPerPage:number,currentDate?:any,oneMonthAgo?:any){
    this.is_sppiner = true
    this.spinner.show()

    let api;
    if (currentDate){
      api = this.dashapi.get_ledger_detail(this.helper.get_local('token'),slug,currentPage,itemsPerPage, currentDate, oneMonthAgo )

    }else{
      api  = this.dashapi.get_ledger_detail(this.helper.get_local('token'),slug,currentPage,itemsPerPage )
    }
  ...
  pageChange(e:any){
    ...
    this.entries_index = []
    this.currentPage  = e
    this.router.navigate(['accounting/ledgers', this.activeSlug,this.currentPage ])
    //  this.get_all_entries(this.activeSlug, this.currentPage, this.itemsPerPage)
