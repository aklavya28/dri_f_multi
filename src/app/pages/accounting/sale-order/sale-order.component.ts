...
export class SaleOrderComponent implements OnInit, OnDestroy {
  ...
  sub_remove_temp_single_product:Subscription | undefined
  ...
  ngOnInit() {
    ...
    this.sub_remove_temp_user_order_items = this.dashapi.remove_temp_user_order_items(token).subscribe((res) =>{
  ...
  apply_discount(){
      ...
      let discouned_total = (Number(el.no_of_unit) * Number(el.unit_price) - cal_discount)
      let cal_gst = discouned_total * Number(el.gst_percent)/100
...
assetsNameValidations(){
  this.saleOrderForm.get('fixedAsset.name')?.setValidators([Validators.required, CustomValidation.valueSelected(this.fixedAssetsApi )])
...
removeProduct(index:number, slug:string){
    ...
    if( credit_from_array.length <= 1 && credit_from_array.includes('debtor')){
     ...
     databoj.push(data)
      ...
      let data = {
        ...
        vechile_no: this.saleOrderForm.get('vehical_no')?.value,
      ...
      if( data?.bill_calculations?.grand_total === ledger_amount ||  this.saleOrderForm.get('credit_fromInputs')?.value.length < 1){
        // console.log("hai", data?.bill_calculations?.grand_total, ledger_amount )
          this.is_sppiner  = true
          this.spinner.show()
          this.sub_save_sale_order = this.dashapi.save_sale_order(token, {data}).subscribe((res:any)=>{
              this.is_sppiner  = false
              this.spinner.hide()
              Swal.fire({
                position: "top-end",
  ...
  ngOnDestroy(): void {
    ...
    this.sub_remove_temp_single_product?.unsubscribe()
  ...
  private _filter3(value: any): any{
    const filterValue = typeof value === 'object' ?  value.name.toLowerCase() : value.toLowerCase();
      ...
      // console.log("dfsdffd", this.khata)
