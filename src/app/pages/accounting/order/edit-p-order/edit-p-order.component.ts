...
@Component({
  selector: 'app-edit-p-order',
  templateUrl: './edit-p-order.component.html',
  styleUrl: './edit-p-order.component.scss',
  animations: [
    trigger(
      'enterAnimation', [
        transition(':enter', [
...
export class EditPOrderComponent implements OnInit,OnDestroy{
...
sub_get_product_categories:Subscription | undefined
 ...
 // autocomplete
  ...
  addDiscountV(): void {
    this.newOrderForm.get('orver_all_discount')?.setValidators([Validators.required,Validators.max(50), CustomValidation.only_digit()])
    this.newOrderForm.updateValueAndValidity();
  }
  ...
  ApiSubmit( ){
        ...
        this.sub_new_order = this.dashapi.update_purchase_order(token, {data}).subscribe({
          next: (res)=>{
            Swal.fire({
              position: "top-end",
              icon: "success",
          ...
          error:(err) =>{
            this.api_errors = []
            console.error(err.error?.err_message)
  ...
  ngOnDestroy(): void {
   ...
   apiOldProducts(token:string, slug:any){
    ...
    this.dashapi.edit_order_with_detail(token, slug).subscribe((res) =>{
       this.services_items = this.oService.jsonServiceBinding(res.body.services)
       this.item_data = this.oService.jsonProductBinding(res.body.product)
       this.fixedAssets = this.oService.jsonFixedBinding(res.body.assets)
        // fetchecing old form inputs
      let order = res.body.data

      if(Number(order.discount_percent) > 0){
        this.newOrderForm.get('is_discount')?.setValue(true)
        this.is_discount = true
        this.newOrderForm.get('orver_all_discount')?.setValue(Number(order.discount_percent))
        this.discount_val = Number(order.discount_percent)
        // this.apply_discount()
      }
      if(Number(order.round_off_amount) !== 0){
        this.newOrderForm.get('round_off')?.setValue(true)
        this.newOrderForm.get('round_off_amt')?.setValue(order.round_off_amount)
